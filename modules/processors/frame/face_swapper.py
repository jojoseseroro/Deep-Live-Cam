    ...
    try:
        if affine_matrix is not None and bgr_fake is not None:
            try:
                ...
                corners = np.array([[0, 0, 1], [fw, 0, 1], [fw, fh, 1], [0, fh, 1]], dtype=np.float32)
                t = corners @ inv.T
                ...
                if rw > 8 and rh > 8:
                    ...
                    mroi = cv2.warpAffine(fm, roi_aff, (rw, rh), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
                    bin_roi = np.where(mroi > 0.5, np.uint8(255), np.uint8(0))
                    k = max(3, (min(rw, rh) // 20) | 1)
                    bin_roi = cv2.erode(bin_roi, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k, k)))
                    ...
                    if bw > 0 and bh > 0:
                        ...
                        if mx1 > 0 and my1 > 0 and mx2 < w - 1 and my2 < h - 1:
                            ...
                            np.copyto(swapped_frame[my1:my2 + 1, mx1:mx2 + 1], blended[my1:my2 + 1, mx1:mx2 + 1], where=mask[my1:my2 + 1, mx1:mx2 + 1, None].astype(bool))
        ...
        np.copyto(swapped_frame[ry0:ry1, rx0:rx1], blended[ry0:ry1, rx0:rx1], where=roi_mask[:, :, None].astype(bool))
        return swapped_frame
    except Exception:
        return swapped_frame
...
def _resolve_model_entry_name(manager) -> Optional[str]:
    if manager.list_models().get(MODEL_NAME):
        return MODEL_NAME
    for entry_name, entry in manager.list_models().items():
...
def pre_check() -> bool:
    ...
    manager = get_manager()
    model_entry_name = _resolve_model_entry_name(manager)
    if not model_entry_name:
        update_status(
            f"{MODEL_NAME} model unavailable: manifest missing entry for name '{MODEL_NAME}' or filename '{MODEL_FILENAME}'",
            NAME,
        )
        return False

    status, message = manager.ensure_model(model_entry_name)
    if status in ('available', 'downloaded'):
        update_status(f"{model_entry_name} model ready: {message}", NAME)
        return True

    update_status(f"{model_entry_name} model unavailable: {message}", NAME)
    return False
...
_cuda_graph_session = {'session': None, 'io_binding': None, 'ort_input': None, 'ort_latent': None, 'recorded': False}
...
def swap_face(source_face: Face, target_face: Face, temp_frame: Frame) -> Frame:
    ...
    if getattr(modules.globals, "poisson_blend", False):
        swapped_frame = _apply_poisson_blend(swapped_frame, original_frame, target_face, M, bgr_fake)
    if opacity >= 1.0:
...
def get_faces_optimized(frame: Frame, use_cache: bool = True) -> Optional[List[Face]]:
    ...
    faces = get_many_faces(frame) if modules.globals.many_faces else ([get_one_face(frame)] if get_one_face(frame) else None)
...
def process_frames(source_path: str, temp_frame_paths: List[str], progress: Any = None) -> None:
    ...
    for temp_frame_path in temp_frame_paths:
        ...
        result_frame = None
        try:
            result_frame = process_frame_v2(temp_frame, temp_frame_path) if use_v2 else process_frame(source_face, temp_frame)
...
def draw_mouth_mask_visualization(frame: Frame, face: Face, mouth_mask_data: tuple) -> Frame:
    ...
    label_pos_x = min_x
    try:
        cv2.putText(vis_frame, "Mouth Mask", (label_pos_x, label_pos_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)
    except Exception:
        pass
    return vis_frame
...
def apply_mouth_area(frame: np.ndarray, mouth_cutout: np.ndarray, mouth_box: tuple, face_mask: np.ndarray, mouth_polygon: np.ndarray) -> np.ndarray:
    ...
    try:
        ...
        if roi.size == 0:
            return frame
        resized_mouth_cutout = mouth_cutout if roi.shape[:2] == mouth_cutout.shape[:2] else gpu_resize(mouth_cutout, (box_width, box_height), interpolation=cv2.INTER_LINEAR)
        ...
        feathered_mask = feathered_mask / max_val if max_val > 1e-6 else np.zeros_like(feathered_mask)
        if len(frame.shape) == 3 and frame.shape[2] == 3:
            mask_3ch = feathered_mask[:, :, np.newaxis].astype(np.float32)
            inv_mask = 1.0 - mask_3ch
            blended_roi = (resized_mouth_cutout.astype(np.float32) * mask_3ch + roi.astype(np.float32) * inv_mask)
...
def apply_color_transfer(source, target):
    ...
    try:
        ...
        return (result_bgr_float * 255.0).astype("uint8")
    except Exception:
        return source
